...
`pyqlite` is a standalone, standard-library-only Python pricing kernel for a deliberately small front-office scope:
...
env PYTHONPYCACHEPREFIX=/private/tmp/pyqlite-pyc python3 -m compileall -q pyqlite pyqlite_xlwings tests examples/pyqlite_workbook.py
```

## Excel Usage With xlwings

The package includes a thin `pyqlite_xlwings` wrapper for xlwings. `pyqlite` remains the standalone pricing library; `pyqlite_xlwings` only adapts selected functions to Excel.

Install the package, including the optional Excel dependency, into the Python environment used by Excel:

```bash
python -m pip install "/path/to/pyqlite_standalone[excel]"
```
...
```text
PYTHONPATH: /path/to/pyqlite_standalone
UDF Modules: pyqlite_xlwings.functions
```

Then click `Import Python UDFs`. The following worksheet functions become available:

```text
PYQL_FX_FORWARD_RATE(evaluation_date, maturity_date, spot_fx, source_rate, target_rate, settlement_days)
PYQL_FX_FORWARD_ZERO_NPV_RATE(evaluation_date, maturity_date, spot_fx, source_rate, target_rate, settlement_days)
PYQL_FX_FORWARD_NPV(evaluation_date, maturity_date, spot_fx, source_nominal, contract_forward_rate, source_rate, target_rate, pay_source_currency, settlement_days)
PYQL_FX_OPTION_NPV(option_type, evaluation_date, expiry_date, spot_fx, strike, foreign_notional, domestic_rate, foreign_rate, volatility)
PYQL_DIGITAL_FX_OPTION_NPV(option_type, evaluation_date, expiry_date, spot_fx, strike, notional, cash_payoff, domestic_rate, foreign_rate, volatility)
PYQL_FRA_NPV(evaluation_date, value_date, maturity_date, notional, strike_rate, forward_rate, discount_rate, long_position, settlement_days)
PYQL_FIXED_BOND_NPV(evaluation_date, maturity_date, face_amount, coupon_rate, discount_rate, frequency_months, settlement_days)
PYQL_CAPFLOOR_NPV(cap_floor_type, evaluation_date, start_date, maturity_date, notional, cap_rate, floor_rate, forward_rate, discount_rate, volatility, frequency_months)
...
```text
=PYQL_FX_FORWARD_RATE(DATE(2026,5,22),DATE(2027,5,24),1.35,0.04,0.025,2)
=PYQL_FX_FORWARD_ZERO_NPV_RATE(DATE(2026,5,22),DATE(2027,5,24),1.35,0.04,0.025,2)
=PYQL_FX_FORWARD_NPV(DATE(2026,5,22),DATE(2027,5,24),1.35,1000000,1.36,0.04,0.025,TRUE,2)
=PYQL_FX_OPTION_NPV("CALL",DATE(2026,5,22),DATE(2027,5,22),1.25,1.27,1000000,0.04,0.02,0.15)
=PYQL_DIGITAL_FX_OPTION_NPV("CALL",DATE(2026,5,22),DATE(2027,5,22),1.25,1.27,1000000,1,0.04,0.02,0.15)
=PYQL_FRA_NPV(DATE(2026,5,22),DATE(2026,11,24),DATE(2027,5,24),1000000,0.0325,0.035,0.035,TRUE,2)
=PYQL_FIXED_BOND_NPV(DATE(2026,5,22),DATE(2029,5,22),100,0.05,0.04,12,2)
...
```python
pyqlite_xlwings.runpython.price_active_sheet
```
...
1. Save a workbook as `pyqlite_workbook.xlsx`.
2. Copy `examples/pyqlite_workbook.py` into the same folder as the workbook.
...
The `Run main` button calls `main()` in the Python file with the same base name as the workbook. The example `main()` delegates to `pyqlite_xlwings.runpython.price_active_sheet`.
...
Internally, `pyqlite` now includes:
...
`PYQL_SWAPTION_NPV` still accepts a plain scalar Black volatility:
